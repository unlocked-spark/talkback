/*
 * Command line control of the reader, carried over from TalkBack for Developers
 * (github.com/qbalsdon/talkback, Apache 2.0), which is where this idea and most of this code
 * come from. It is here because a screen reader used as a measuring instrument has to be told
 * where to go: synthetic touch events do not drive one, since gestures carry timing and
 * multi finger thresholds that adb input cannot reproduce.
 *
 * What it controls is the reader's own navigation, not the touch layer. The receiver asks the
 * service to run the action a gesture is bound to, so it answers what the reader reads and in
 * what order, and says nothing about whether an application's own views break exploration by
 * touch. That question needs multi touch injection on real hardware.
 */
